...
async function registerOperatorWithAVS() {
  ...
  const serviceManagerAddress =
    helloWorldDeploymentData.addresses.helloWorldServiceManager
  ...
  const stakeRegistryABI = JSON.parse(
    fs.readFileSync(
  ...
  try {
    ...
    const operatorRegistrationDigestHash =
    ...
    const signature = await wallet.signMessage(
      ethers.getBytes(operatorRegistrationDigestHash)
    )
    ...
    const tx = await stakeRegistry.registerOperatorWithSignature(
      operatorSignature,
      signingKeyAddress
    )
    await tx.wait()

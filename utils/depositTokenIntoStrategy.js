...
async function depositTokenIntoStrategy(tokenAddress, amount) {
  ...
  const strategyManagerABI = JSON.parse(fs.readFileSync(path.resolve(__dirname, '../abis/IStrategyManager.json'), 'utf8'));

  const strategyFactoryAddress = coreDeploymentData.addresses.strategyFactory;
  const strategyFactoryABI = JSON.parse(fs.readFileSync(path.resolve(__dirname, '../abis/IStrategyFactory.json'), 'utf8'));
  const tokenABI = JSON.parse(fs.readFileSync(path.resolve(__dirname, '../abis/ERC20Mock.json'), 'utf8'));

  const strategyManager = new ethers.Contract(strategyManagerAddress, strategyManagerABI, wallet);
  ...
  try {
    ...
    const depositTx = await strategyManager.depositIntoStrategy(strategy, tokenAddress, amount);
